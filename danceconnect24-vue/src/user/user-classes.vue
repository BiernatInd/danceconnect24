    ...
    async getEvents() {
      ...
      try {
        ...
        if (response.data) {
          ...
          // Aktualizacja dni w miesiącu na podstawie odpowiedzi z API
          this.daysInCurrentMonth = this.daysInCurrentMonth.map((day) => {
            if (day.dayOfMonth) {
              const formattedDate = `${year}-${month}-${String(
                day.dayOfMonth
              ).padStart(2, "0")}`;
    ...
    async getClasses(day) {
      if (day.dayOfMonth) {
        ...
        console.log({
          ...
          month: month,
          year: year,
          user_id: localStorage.getItem("userId"), // Dodajemy userId do zapytania
        });
        ...
        try {
          ...
          // Przefiltruj klasy na podstawie daty
          ...
          console.log("Filtered Classes Data:", filteredClasses);

          this.classesData = filteredClasses; // Dodanie przefiltrowanych klas do stanu
