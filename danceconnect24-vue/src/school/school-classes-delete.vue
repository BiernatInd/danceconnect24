<template>
        ...
        >
          ...
          <div class="reservation-item-button">
            <button
              ...
              @click="showDelete(reservation.id)"
            >
              Delete classes
            </button>
          </div>
  ...
  <div class="confirmation" :class="{ active: isConfirmationActive }">
    <div class="confirmation-content">
      ...
      <p>Are you sure you want to delete your classes?</p>
...
export default {
  ...
  data() {
    return {
      ...
      selectedInstructor: "",
      selectedClassType: "",
      selectedClassId: null, // Dodajemy zmienną do przechowywania wybranego ID klasy
  ...
  methods: {
    ...
    showDelete(classId) {
      this.selectedClassId = classId; // Ustawienie ID klasy do usunięcia
      this.isConfirmationActive = true; // Aktywacja okna potwierdzenia
    ...
    async confirmDelete() {
      ...
      if (this.selectedClassId) {
        try {
          // Logowanie danych wysyłanych w zapytaniu
          const payload = { classId: this.selectedClassId };
          console.log("Payload being sent:", payload);

          const response = await axios.post(
            "http://localhost:8000/school/delete",
            payload
          );
          ...
          this.closeReservations(); // Zamknięcie sekcji rezerwacji
          this.getEvents(); // Odświeżenie wydarzeń po usunięciu klasy
          this.isConfirmationActive = false; // Zamknięcie okna potwierdzenia
        } catch (error) {
          console.error(
    ...
    hideConfirmation() {
      this.isConfirmationActive = false; // Zamknięcie okna potwierdzenia bez usuwania
      this.selectedClassId = null; // Resetowanie ID klasy
    },
    formatTime(time) {
